...
from .serializers import (ActivityLogSerializer, ChecklistItemCreateSerializer,
                          ChecklistItemSerializer, SprintCreateSerializer,
                          SprintSerializer, TaskCommentSerializer,
                          TaskCreateSerializer, TaskRelationCreateSerializer,
                          TaskRelationSerializer, TaskSerializer, TaskTreeSerializer,
                          TimeEntrySerializer)
...
class SprintDetailView(APIView):
    def _sprint(self, slug, pid, sid, user):
        return get_object_or_404(Sprint, id=sid, project__id=pid,
...
class SprintStatsView(APIView):
    def get(self, request, workspace_slug, project_id, sprint_id):
        sprint = get_object_or_404(Sprint, id=sprint_id, project__id=project_id,
        ...
        return Response({"total": total, "done": done, "remaining": remaining,
...
class TaskListCreateView(APIView):
    ...
    def get(self, request, workspace_slug, project_id):
        ...
        if q.get("assignee"): qs = qs.filter(assignee__id=q["assignee"])
        if q.get("sprint"):   qs = qs.filter(sprint__id=q["sprint"])
        if q.get("search"):   qs = qs.filter(title__icontains=q["search"])
...
class TaskDetailView(APIView):
    ...
    def get(self, request, workspace_slug, project_id, task_id):
        return Response(TaskSerializer(self._task(workspace_slug, project_id, task_id, request.user)).data)
...
class TaskCommentListCreateView(APIView):
    ...
    def get(self, request, workspace_slug, project_id, task_id):
        task = self._task(workspace_slug, project_id, task_id, request.user)
        return Response(TaskCommentSerializer(task.comments.select_related("author"), many=True).data)
...
class TaskActivityView(APIView):
    def get(self, request, workspace_slug, project_id, task_id):
        task = get_object_or_404(Task, id=task_id, project__id=project_id,
                                 project__workspace__slug=workspace_slug,
                                 project__members__user=request.user)
        return Response(ActivityLogSerializer(task.activity.select_related("actor")[:30], many=True).data)
...
class ChecklistItemDetailView(APIView):
    def _item(self, slug, pid, tid, iid, user):
        return get_object_or_404(ChecklistItem, id=iid, task__id=tid,
                                 task__project__id=pid, task__project__workspace__slug=slug,
...
class TaskRelationListCreateView(APIView):
    ...
    def get(self, request, workspace_slug, project_id, task_id):
        task = self._task(workspace_slug, project_id, task_id, request.user)
        qs = list(task.relations_from.select_related("from_task", "to_task")) + \
             list(task.relations_to.select_related("from_task", "to_task"))
...
class TimeEntryDetailView(APIView):
    def delete(self, request, workspace_slug, project_id, task_id, entry_id):
        entry = get_object_or_404(
            TimeEntry, id=entry_id, task__id=task_id,
...
class AnalyticsView(APIView):
    def get(self, request, workspace_slug, project_id):
        get_object_or_404(Project, id=project_id, workspace__slug=workspace_slug,
